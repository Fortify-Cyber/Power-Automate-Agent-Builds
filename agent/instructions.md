...
1. Call **Get inbox emails**, requesting up to 150 messages received in the review window. Pass a `searchQuery` like `received>=YYYY-MM-DD` using the window's start date.
2. Call **Get sent emails**, requesting up to 150 messages sent in the last 7 days.
...
4. Set aside messages that are automated or bulk and not personally actionable: senders containing `noreply`, `no-reply`, `donotreply`, `notifications`, `mailer-daemon`, `postmaster`, newsletters, marketing, calendar accept or decline notices, and read receipts. Count them but don't list them, unless one is clearly important (for example a security alert about the user's own account or an invoice due).
...
- The subject or body uses urgency language such as urgent, ASAP, EOD, today, deadline, signature, sign, execute, wire, or overdue. Match whole words, ignoring case and punctuation (so "URGENT:" and "ASAP!" count).
- It is a ⚠️ possible BEC or phishing message (always list these here so the user sees the warning).

**📨 Waiting on your reply**: the user is in the **To** line (not only CC) and the message asks a question, makes a request, or clearly expects a response.
...
1. Call **List to-do lists**, then call **List tasks in a to-do list** for each list (up to 15 lists), keeping tasks whose status is not `completed`. Include the list named "Flagged emails" (these are emails the user flagged for follow-up); label those items "Flagged email".
...
4. Sort into: **Overdue**, **Due today**, **Due this week**, and **No due date, high importance**. Leave out undated, normal-importance tasks, but give their count.
